...
class ImportOneCPackage extends ImportPattern
{
    ...
    protected function modifyEntity($item)
    {
		...
		{
			$r = $collision->resolve($item);
		}

        if(!$criterion->equals($fields) ||
            ($criterion->equals($fields) && !$r->hasWarnings()))
        {
            $r = $item->import($params);
        }
    ...
    protected function import(array $items)
    {
        ...
        foreach($items as $item)
        {
            if($item->getOwnerTypeId() == EntityType::PROFILE)
            {
                ...
                {
                    $result->addError(new Error('Person type is not load'));
                }

                if(empty($userId))
                {
                    $result->addError(new Error('User id is not load'));
                }
                ...
                if($r->isSuccess())
                {
                    if($item->getOwnerTypeId() == EntityType::SHIPMENT)
                    {
                        if(!$isShipped && $order->isShipped())
                            $r = $this->onAfterShipmentModifyChangeStatusOnDelivery($itemOrder);
                    }
                }
            }
    ...
    protected function onBeforeShipmentCollectionModify(Exchange\Entity\OrderImport $orderImport, $items)
    {
        ...
        if(!empty($shipmentList))
        {
            foreach($shipmentList as $id=>$shipment)
            {
                ...
                if($collision->resolve($item)->isSuccess())
                {
                    $result = $item->delete();
                }
                else
                {
                    $result->addWarning(new ResultError(GetMessage('SALE_EXCHANGE_PACKAGE_ERROR_ORDER_CANNOT_UPDATE'), "PACKAGE_ERROR_ORDER_CANNOT_UPDATE"));
                }
    ...
    protected function onBeforePaymentCollectionModify(Exchange\Entity\OrderImport $orderImport, $items)
    {
        ...
        if(!empty($paymentList))
        {
            foreach($paymentList as $id=>$payment)
            {
                ...
                {
                    $result->addError(new Error('', 'EXCHANGE_ERROR'));
                    $result->addWarning(new ResultError(GetMessage('SALE_EXCHANGE_PACKAGE_ERROR_ORDER_CANNOT_UPDATE'), "PACKAGE_ERROR_ORDER_CANNOT_UPDATE"));
                }
    ...
    protected function onBeforeEntityModify(Exchange\Entity\OrderImport $orderImport, array $items)
    {
		...
		{
			$result->addErrors($basketItemsResult->getErrors());
			$result->addWarnings($basketItemsResult->getErrors());
		}
		if($basketItemsResult->hasWarnings())
		{
			$result->addWarnings($basketItemsResult->getWarnings());
		}
		...
		if($paymentResult->hasWarnings())
		{
			$result->addWarnings($paymentResult->getWarnings());
		}
		...
		if($shipmentResult->hasWarnings())
		{
			$result->addWarnings($shipmentResult->getWarnings());
		}
    ...
    protected function onAfterShipmentModifyChangeStatusOnDelivery(Exchange\Entity\OrderImport $orderImport)
    {
        ...
        if($order->isShipped())
        {
            /** @var OneC\ImportSettings $settings */
	...
	private function UpdateCashBoxChecks(Exchange\Entity\OrderImport $orderImport, array $items)
	{
					...
					if(isset($params['CASH_BOX_CHECKS']))
					{
						...
						if ($fields['ID'] > 0)
						{
							...
							if ($data = $res->fetch())
							{
								if ($data['STATUS'] <> 'Y')
								{
									$applyResult = Cashbox1C::applyCheckResult($fields);
		...
		$c = $collision->resolve($orderImport);

		if(!$c->isSuccess() && $bCheckUpdated)
		{
			return $result; //true
		}
		else
