...
//		add only checked photos
		$checkedPhotos = Vk\PhotoResizer::checkPhotos($data["PICTURE"], 'ALBUM');
		if ($checkedPhotos)
...
//		add item to log, if image was be resized
		if ($checkedPhotos['RESIZE_UP'])
			$logger->addError('ALBUM_PHOTOS_RESIZE_UP', $data["ID"]);
		if ($checkedPhotos['RESIZE_DOWN'])
			$logger->addError('ALBUM_PHOTOS_RESIZE_DOWN', $data["ID"]);
