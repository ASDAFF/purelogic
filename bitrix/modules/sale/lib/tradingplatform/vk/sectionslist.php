...
class SectionsList
{
	 ...
	 */
	public static function getSections($tree = false)
	{
...
//			parse ITEMS for current iblock
	...
	/**
	 * Parse sections list and find nearest parent which have export settings.
	 * If there parent is not exist - return false
	 *
	 * @param $sectionId
	 * @return bool
	 */
	public function getParentSection($sectionId)
	{
//		get ID for all parents sections
		$resParentsIds = \CIBlockSection::GetNavChain(0, $sectionId, array("ID"));
		$parentsIds = array();
		while ($item = $resParentsIds->Fetch())
		{
			$parentsIds[$item["ID"]] = $item["ID"];
		}
//		we not need current section
		unset($parentsIds[$sectionId]);

//		get minimal parent with settings
		$nearestParent = NULL;
		foreach ($parentsIds as $parentId)
		{
			$parent = $this->mappedSections[$parentId];
			if ($parent["PARAMS"]["INHERIT"] === false)
				$nearestParent = $parent["BX_ID"];
		}
		
		if ($nearestParent)
			return $this->mappedSections[$nearestParent];
		else
			return false;
	}
