...
//				set flag if image was be resized
				if ($photoChecked['RESIZE'])
					$result[$photoChecked['RESIZE']] = true;
			}
		}
...
//			save new photo
			$paramsToSave = array(
				'name' => $photoParams['FILE_NAME'],
				'del' => 'Y',
				'content' => file_get_contents($photoUrl),
			);
			
			$photoId = \CFile::SaveFile($paramsToSave, "vk_export_resize_img");
		}
			...
			if ($arSize["width"] > 0 && $arSize["height"] > 0)
			{
				switch ($resizeType)
				{
					case BX_RESIZE_IMAGE_EXACT:
						...
						$arSourceSize["x"] = $x;
						$arSourceSize["y"] = $y;
						$arSourceSize["width"] = ceil($arSize["width"] / $ratio, 0);
						$arSourceSize["height"] = ceil($arSize["height"] / $ratio, 0);
						break;
