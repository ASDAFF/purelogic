...
final class ReportManager
{
	...
	public static function getPrintableZReport($cashboxId)
	{
		...
		if ($lastZReport && $lastZReport['STATUS'] != 'Y')
		{
		...
		else
		{
			...
			$prevPrintDate->setTime($cashbox['SETTINGS']['Z_REPORT_TIME']['H'], $cashbox['SETTINGS']['Z_REPORT_TIME']['M']);
	...
	public static function saveZReportPrintResult($reportId, $data)
	{
		...
		$report = CashboxZReportTable::getRowById($reportId);
		if (!$report)
		{
		...
		if (isset($data['ERROR']))
		{
			...
			if ($report && $report['CASHBOX_ID'] > 0)
				Manager::writeToLog(array('CASHBOX_ID' => $report['CASHBOX_ID'], 'MESSAGE' => $errorMessage));

			CashboxZReportTable::update($reportId, array('STATUS' => 'E', 'DATE_PRINT_END' => new DateTime()));
		}
		else
		{
			$updateResult = CashboxZReportTable::update(
				$reportId,
				array(
					'STATUS' => 'Y',
