...
final class CheckManager
{
		...
		if (!$cashboxList)
		{
			...
			if ($dbRes->fetch())
			{
				foreach ($entities as $entity)
				{
					...
					{
						$r = new Result();
						$r->addWarning(new Error(Loc::getMessage('SALE_CASHBOX_NOT_FOUND')));
		...
		if ($saveResult->isSuccess())
		{
			...
			foreach ($cashboxList as $item)
			{
				...
				if ($cashbox instanceof IPrintImmediately)
				{
					...
					{
						CashboxCheckTable::update($checkId, array('STATUS' => 'P', 'DATE_PRINT_START' => new Type\DateTime(), 'CASHBOX_ID' => $item['ID']));
						return $result;
					}
					else
					{
	...
	public static function savePrintResult($checkId, array $data)
	{
		...
		if (isset($data['ERROR']))
		{
			...
			if ($check && $check['CASHBOX_ID'] > 0)
				Manager::writeToLog(array('CASHBOX_ID' => $check['CASHBOX_ID'], 'MESSAGE' => $data['ERROR']['MESSAGE']));

			if ($data['NEED_MARKED'] === true)
			{
				CashboxCheckTable::update($checkId, array('STATUS' => 'E', 'DATE_PRINT_END' => new Main\Type\DateTime()));
