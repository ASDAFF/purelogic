...
$headers = array(
	...
	array("id" => "NUMBER_FN", "content" => GetMessage("SALE_CASHBOX_NUMBER_FN"), "sort" => "NUMBER_FN", "default" => true),
	...
	array("id" => "ENABLED", "content" => GetMessage("SALE_CASHBOX_IS_ONLINE"), "sort" => "ENABLED", "default" => true),
...
while ($cashbox = $dbResultList->Fetch())
{
	...
	$row->AddField("NUMBER_FN", htmlspecialcharsbx($cashbox['NUMBER_FN']));
	...
	if ($saleModulePermissions >= "W")
	{
		...
		$arActions[] = array(
			...
			"ACTION" => "if(confirm('".GetMessage('SALE_CONFIRM_DEL_MESSAGE')."')) ".$lAdmin->ActionDoGroup($cashbox['ID'], "delete"),
		);
	}
...
if ($saleModulePermissions == "W")
{
	...
	$aContext = array(
		array(
			...
			"TITLE" => GetMessage("SALE_CASHBOX_ADD_NEW_ALT"),
			"LINK" => '#',
			"ICON" => "btn_new",
