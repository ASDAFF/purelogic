...
if($arResult["ERROR"] === '' && $saleModulePermissions >= "W" && check_bitrix_sessid())
{
	...
	switch ($action)
	{
		...
		case "generate_link":
			...
			break;		
		case "get_default_kkm_settings":
			
			$typeId = (int)$request->get('kkmTypeId');
			$currentId = (int)$request->get('kkmCurrentId');
			if ($currentId == $typeId)
			{
				$currentKkmData = \Bitrix\Sale\Cashbox\Internals\CashboxTable::getList(
					array(
						'filter' => array('KKM_ID' => $currentId)
					)
				);
				if ($currentType = $currentKkmData->fetch())
				{
					$arResult["SETTINGS"] = $currentType['SETTINGS'];
					if (!empty($currentType['SETTINGS']))
					{
						$arResult["USER_SETTINGS"] = true;
						break;
					}
				}
			}
			$typeData = \Bitrix\Sale\Cashbox\Internals\KkmModelTable::getById($typeId);
			if ($type = $typeData->fetch())
			{
				$arResult["SETTINGS"] = $type['SETTINGS'];
			}
			else
			{
				$arResult["ERROR"] = "Error! Wrong kkm type id!";
			}			
			break;
		default:
