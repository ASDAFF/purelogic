...
(function(window) {
	...
	BX.Sale.Cashbox = {
		...
		toggleSettings: function(type)
		{
			if (type == '\\Bitrix\\Sale\\Cashbox\\Cashbox1C' || BX('KKM_ID').value == '' || type == '')
			{
				tabControl.DisableTab('edit3');
			}
			else
			{
				tabControl.EnableTab('edit3');
			}
		},

		changeDefaultSettings: function(kkmType, currentType)
		{
			var vatInputs = BX('cashbox-settings-vat').getElementsByTagName('input');
			var paymentInputs = BX('cashbox-settings-payment-type').getElementsByTagName('input');

			this.toggleSettings(BX('HANDLER').value);
			BX.ajax({
				data: {
					'action': 'get_default_kkm_settings',
					'kkmTypeId': kkmType || null,
					'kkmCurrentId': currentType || null,
					'sessid': BX.bitrix_sessid()
				},
				method: 'POST',
				dataType: 'json',
				...
				onsuccess: BX.delegate(function(result)
					{
						...
						if (result)
						{
							if (!result.ERROR)
							{
								...
								Array.prototype.forEach.call(vatInputs, function(element)
								{
									if (settings === null || kkmType == '')
									{
										element.value = null;
									}
									else
									{
										var vatValue = element.getAttribute('bx-vat-value');

										if (currentType == kkmType && result.USER_SETTINGS == true)
										{
											element.value = settings.VAT[element.getAttribute('bx-vat-id')];
										}
										else
										{
											var defaultKeyVat = (vatValue != 'NOT_VAT') ? parseInt(vatValue) : vatValue;

											if (vatValue == defaultKeyVat)
											{
												var defaultVat = settings.VAT[defaultKeyVat];
												if (defaultVat !== undefined)
												{
													element.value = defaultVat;
												}
												else
												{
													element.value = null;
								...
								Array.prototype.forEach.call(paymentInputs, function(element)
								{
									if (settings === null || kkmType == '')
									{
										element.value = null;
									...
									{
										var paymentType = element.getAttribute('bx-payment-type');
										var defaultVat = settings.PAYMENT_TYPE[paymentType];
										if (defaultVat !== undefined)
										{
											element.value = defaultVat + 1;
										}
										else
										{
											element.value = null;
				...
				onfailure: function() {BX.debug('onfailure: changeDefaultSettings');}
			});
		},
