		...
		<script>
			...
			if(!BX.HandlerMoneyField) {
				...
				BX.HandlerMoneyField.prototype.changeCurrency = function() {
					...
					if(this.realValue)
					{
						this.realValue = this.realValue.replace(new RegExp(this.oldFormat.thousandsSep, 'g'),'');
						this.input.value = this.realValue.replace(this.oldFormat.decPoint,this.decPoint);
					}
					this.setRealValue();
				...
				BX.HandlerMoneyField.prototype.checkFormatValue = function() {
					...
					var regExp = new RegExp(
						'^\\d{1,3}('+this.thousandsSep+'?\\d{3})*(\\'+this.decPoint+'\\d{1,'+this.decimals+'})?$');
				...
				BX.HandlerMoneyField.prototype.setHiddenValue = function() {
					if (this.input.value)
					{
						this.hidden.value = this.realValue.replace(new RegExp(this.thousandsSep, 'g'), '') +
