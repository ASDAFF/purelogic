...
if($_SERVER["REQUEST_METHOD"]=="POST" && $_POST["FEEDBACK_FORM_".$ALX] && check_bitrix_sessid())
{
					...
					if($arParams["PROPS_AUTOCOMPLETE_VETO"]=="Y" && $USER->IsAuthorized())
					{
						...
						)
						{
							if(in_array($k, $arParams["PROPS_AUTOCOMPLETE_EMAIL"]))
								$PROPS[$k] = htmlspecialcharsbx($USER->GetEmail());
						}
	...
	$arResult["FEEDBACK_TEXT"] = trim(htmlspecialcharsbx($_POST["FEEDBACK_TEXT_".$ALX]));
	...
	if(count($arResult["FORM_ERRORS"]) <= 0)
	{
		...
		if($arParams['ALX_CHECK_NAME_LINK']!='Y' && empty($arResult["FORM_ERRORS"]))
		{
	...
	if($arrProp["PROPERTY_TYPE"] == "G")
	{
		...
		if(isset($arrProp["LINK_IBLOCK_ID"]))
		{
			$resSect = CIBlockSection::GetList(
				...
				false,
				Array("IBLOCK_ID", "ID", "ACTIVE", "NAME", "DEPTH_LEVEL")
			);

			while($arSection = $resSect->Fetch())
			{
