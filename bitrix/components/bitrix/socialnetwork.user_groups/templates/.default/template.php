...
else
{
	...
	)
	{
		...
		$menuItems[] = array(
			...
			"ID" => "workgroups_all",
			"IS_ACTIVE" => (!$myTabActive && !$arResult["filter_my"] && !$arResult["filter_archive"] && !$arResult["filter_extranet"] && !$arResult["filter_tags"])
		);
				...
				foreach ($arResult["Groups"]["List"] as $group)
				{
					...
					?><div class="sonet-groups-group-block"><?
						...
						?><span class="sonet-groups-group-text"><?
							...
							{
								?><span id="bx-sonet-groups-request-<?=intval($group['GROUP_ID'])?>" class="sonet-groups-group-btn-container"><span class="webform-small-button webform-small-button-transparent"><?=GetMessage('SONET_C33_T_F_DO_REQUEST')?></span></span><?
								?><script>
									BX.bind(BX('bx-sonet-groups-request-<?=intval($group['GROUP_ID'])?>'), 'click', function(e) {
										oSUG.sendRequest({
											action: 'REQUEST',
											groupId: <?=intval($group['GROUP_ID'])?>,
											button: BX('bx-sonet-groups-request-<?=intval($group['GROUP_ID'])?>')
										});
										return BX.PreventDefault(e);
									});
								</script><?
